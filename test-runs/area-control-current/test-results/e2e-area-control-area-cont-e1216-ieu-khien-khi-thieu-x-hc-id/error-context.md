...
- Name: e2e\area-control\area-control.api.spec.ts >> Area Control API Real System TC1-TC19 >> TC18 - Khong dieu khien khi thieu x-hc-id
...
> 722 |       expect(getSlotValue(statuses, deviceId, slot)).toBe(initialValue)
  ...
  809 | }
  810 | 
  811 | const loginAreaControlUser = async (
  812 |   areaControlEnv: AreaControlEnv,
  813 |   username: string,
  814 |   password: string,
  815 | ): Promise<LoginResult> => {
  816 |   if (!username || !password) {
  817 |     throw new Error('Username and password are required for real login')
  818 |   }
  819 | 
  820 |   const loginContext = await request.newContext({
  821 |     baseURL: areaControlEnv.baseUrl,
  822 |     extraHTTPHeaders: commonHeaders(),
```